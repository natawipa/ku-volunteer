  ...
  describe('Create Event/Activity', () => {
    it('should successfully create a new activity', () => {
      ...
      cy.fixture('activities').then((activities) => {
        ...
        // Select categories
        cy.get('[data-testid="category-environment"]').check()
        cy.get('[data-testid="category-community"]').check()
        
        // Upload cover image
        cy.get('[data-testid="cover-image-input"]').selectFile('cypress/fixtures/test-image.jpg', { force: true })
        ...
        // Should redirect to activity details
        cy.url().should('match', /\/events\/\d+/)
        cy.get('[data-testid="activity-title"]').should('contain', activity.title)
      })
    })
