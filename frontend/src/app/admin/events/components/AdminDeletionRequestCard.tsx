...
export default function AdminDeletionRequestCard({ event }: Props) {
  return (
  <div className="relative overflow-hidden rounded-lg shadow-lg p-5 bg-gradient-to-r from-orange-300/25 to-amber-300/25 hover:scale-[1.01] transition-transform duration-200">
      ...
      <div className="relative z-10 flex flex-col gap-2">
        ...
        <div className="pt-2 flex gap-2">
          <a href={`/admin/approve/delete/${event.id}`} className="flex-1 bg-green-600 text-white text-center py-2 rounded-md text-sm hover:bg-green-700 transition-colors">Review</a>
          <button className="bg-red-100 text-red-700 py-2 px-4 rounded-md text-sm hover:bg-red-200 transition-colors">Reject</button>
        </div>
      </div>
    </div>
  );
