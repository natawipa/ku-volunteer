    ...
    >
      ...
      <div className="relative z-10 flex flex-col gap-2">
        <div className="flex items-start justify-between gap-4">
          ...
          data-testid="activity-title" title={activity.title}>{activity.title}</h3>
        ...
        <div className="flex items-center text-sm text-gray-700 gap-2">
          ...
          {activity.start_at === activity.end_at ? (
            <span data-testid="activity-date">{formatDate(activity.start_at)}</span>
          ) : (
            <span data-testid="activity-date">{formatDate(activity.start_at)} - {formatDate(activity.end_at)}</span>
          )}
        </div>
        <div className="flex items-center text-sm text-gray-700 gap-2">
          <MapPinIcon className="w-4 h-4" />
          <span className="truncate" data-testid="activity-location" title={activity.location}>{activity.location}</span>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4 text-xs text-gray-700 mt-1">
          {typeof activity.current_participants === 'number' && typeof activity.max_participants === 'number' && (
            <span className="font-medium" data-testid="activity-participants">{activity.current_participants}/{activity.max_participants} participants</span>
          )}
            {activity.hours_awarded && <span data-testid="activity-hours">{activity.hours_awarded} hrs</span>}
        </div>
