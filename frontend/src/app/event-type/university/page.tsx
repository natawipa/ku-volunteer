...
export default function UniversityActivitiesPage() {
  ...
  // Fetch university activities on component mount
  useEffect(() => {
    const fetchUniversityActivities = async () => {
      try {
        setLoading(true);
        const response = await activitiesApi.getActivities();
        
        if (response.success && response.data) {
          // Filter activities that have "University Activities" category (same logic as homepage)
          const universityActivities = response.data.filter(activity => 
            Array.isArray(activity.categories) && activity.categories.some(cat => 
              cat.includes("University Activities")
            )
          );
          setActivities(universityActivities);
        } else {
          setError(response.error || 'Failed to fetch activities');
        }
      } catch (err) {
        console.error('Error fetching university activities:', err);
    ...
    <div className="relative">
      ...
      <div className="relative p-6"> 
        ...
        <section className="mb-8">
          <div className="bg-gradient-to-r from-[#A1E59E]/26 to-[#5992FF]/26 rounded-lg p-6 flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold mb-2">University Activities</h1>
        ...
        {loading && (
          <div className="flex justify-center items-center h-48">
            ...
            <span className="ml-3 text-gray-600">Loading university activities...</span>
          </div>
        )}
